...
every game currently on the platform, whether it's an OpenSpiel-family game
(tic_tac_toe, repeated_pd, avalon — where each `LegalAction.action_id` is a
stringified move index) or a structured RuntimeAdapter game like Pokémon
(where `action_id` looks like `"move:0"`/`"switch:1"`/`"draft_pick:<id>"`).
...
messaging-enabled games like repeated_pd/avalon just fine — the runtime
auto-terminates each messaging round on its behalf.
